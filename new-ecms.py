...
def main_page(username):
    ...
    # Ensure 'Actual Date' in the dataframe is converted to pd.Timestamp
    combined_df['Actual Date'] = pd.to_datetime(combined_df['Actual Date'])

    # Get min and max dates as pandas Timestamp objects
    min_date = combined_df['Actual Date'].min()
    max_date = combined_df['Actual Date'].max()

    # Convert default start and end dates to pandas Timestamps for consistency
    default_start_date = pd.Timestamp(default_start_date)
